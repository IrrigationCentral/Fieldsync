...
// Send browser notification (for users with app open)
...
const sendNotification = async (user, title, message, options = {}) => {
  if (!user) {
...
const notifyMultiple = async (users, title, message, options = {}) => {
  const results = [];
...
// New job/issue reported - notify managers and office
export const notifyNewIssue = async (managers, officeStaff, job, reportedBy) => {
...
export const notifyJobCompletedFarmer = async (farmer, job) => {
...
export const notifyJobCompletedStaff = async (managers, officeStaff, job, completedBy) => {
...
// High priority alert
export const notifyUrgentIssue = async (managers, job, reportedBy) => {
